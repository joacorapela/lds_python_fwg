...
def main(argv):
    ...
    parser.add_argument("--filtering_params_filename", type=str,
                        default="../../metadata/00000012_smoothing.ini",
    ...
    data = pd.read_csv(filepath_or_buffer=data_filename, header=0)
    ts = data.iloc[:,0]
    ...
    diag_V0_values = np.array(
        [float(diag_value) for diag_value in
         smoothing_params[filtering_params_section]
         ["diag_V0_values"][1:-1].split(",")])
